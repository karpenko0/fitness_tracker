...
### Фаза 5 — Безопасность, качество, производительность (1 день) ✅ ГОТОВО
> Реализовано: `redact-sensitive-data` расширен (password, telegram_webhook_secret, notes/medicalNotes/medicationNotes) + 3 unit-теста; `test/integration/habit-security.api.spec.ts` (9) — 401 без токена/с мусорным токеном/с отозванной сессией через **реальный JwtStrategy** (без override guard), валидный токен проходит; ownership: GET /habits/:id и POST progress скоупятся по userId из токена (чужое → 404), GET /today только свои; 429 — реальный ThrottlerGuard (limit 2 → третий запрос 429). p95: `test/integration/habit-perf.api.spec.ts` (2) — handler-time in-process на моках: GET /habits/today (10 привычек, 300 запросов) p95 ≈ 5 мс, POST progress p95 ≈ 2.4 мс (цель ≤ 300 мс); для стенда с PostgreSQL — `scripts/load-smoke-habits.js` (ACCESS_TOKEN/HABIT_ID/TASK_ID, p50/p95/p99, exit 1 при p95 > 300). Аудит логов: habit-модуль логирует только счётчики, `TelegramBotClient.sanitizeError` вычищает токен (тест), console не используется, `HttpExceptionFilter` прогоняет details через redactSensitiveData. Idempotency-Key на всех записях и 409 IDEMPOTENCY_KEY_REUSED при ином теле — покрыто в Фазах 1–2. Полный прогон 45/45 suites, 320/320 tests.
...
- Создание/управление привычками (10 требований) → Фаза 1 ✅ (включая «пауза/архив не создаёт новых заданий» — проверено в Фазе 2 и E2E-сценарии 7)
...
npx jest                            # 46/46 suites, 330/330 tests
