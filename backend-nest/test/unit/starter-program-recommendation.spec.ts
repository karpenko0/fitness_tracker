...
describe('StarterProgramRecommendationService', () => {
  const exact = { id: 'exact', title: 'Exact', goals: ['WEIGHT_LOSS'], levels: ['BEGINNER'], locations: ['HOME'], workoutsPerWeek: 3, durationMinutes: 30, requiredEquipment: ['DUMBBELLS'], isFallback: false, contraindications: [], workouts: [] };
  const fallback = { id: 'fallback', title: 'Fallback', isFallback: true, contraindications: [], workouts: [] };
  ...
  it('selects the exact compatible published program', async () => {
    ...
    await expect(service.recommend(data)).resolves.toEqual({ program: exact, fallback: false, matchType: 'exact' });
  });
  it('uses the published fallback when no compatible program exists', async () => {
    ...
    await expect(service.recommend(data)).resolves.toEqual({ program: fallback, fallback: true, matchType: 'fallback' });
  });
