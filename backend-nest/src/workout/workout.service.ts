...
export class WorkoutService {
  ...
  private async snapshotProgram(tx: Prisma.TransactionClient, userId: string, workoutId: string, programWorkoutId: string | undefined) { const source = await tx.programWorkout.findFirst({ where: { id: programWorkoutId, program: { assignments: { some: { userId, status: 'ACTIVE' } } } }, include: { exercises: { orderBy: { position: 'asc' }, include: { exercise: true } } } }); if (!source) throw new NotFoundException({ code: 'PROGRAM_WORKOUT_NOT_FOUND', message: 'Program workout was not found' }); await tx.workout.update({ where: { id: workoutId }, data: { title: source.title, programAssignmentId: (await tx.userProgramAssignment.findFirstOrThrow({ where: { userId, programId: source.programId, status: 'ACTIVE' } })).id } }); for (const item of source.exercises) { const exercise = await tx.workoutExercise.create({ data: { workoutId, position: item.position, title: item.exercise.title, muscleGroup: item.exercise.muscleGroup, equipment: item.exercise.equipment, techniqueUrl: item.exercise.techniqueUrl, catalogExerciseId: item.exerciseId } }); for (const [index, set] of (item.plannedSets as any[]).entries()) await tx.workoutSet.create({ data: { exerciseId: exercise.id, position: index + 1, setType: set.setType || 'WORKING', plannedReps: set.reps ?? null, plannedWeightKg: set.weightKg ?? null, restSeconds: set.restSeconds ?? 0 } }); } }
