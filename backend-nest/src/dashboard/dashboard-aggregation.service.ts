...
export class DashboardAggregationService {
  ...
  private volume(exercises: Array<{ kind: WorkoutExerciseKind; sets: Array<{ status: string; reps: number | null; weightKg: unknown }> }>) { return exercises.filter(exercise => exercise.kind === 'STRENGTH').reduce((total, exercise) => total + exercise.sets.filter(set => set.status === 'COMPLETED' && set.reps != null && set.weightKg != null).reduce((subtotal, set) => subtotal + Number(set.weightKg) * set.reps!, 0), 0); }
